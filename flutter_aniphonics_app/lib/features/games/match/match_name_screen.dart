...
class MatchNameScreen extends StatefulWidget {
  const MatchNameScreen({super.key});

  @override
  State<MatchNameScreen> createState() => _MatchNameScreenState();
}
...
class _MatchNameScreenState extends State<MatchNameScreen> {
  ...
  void initState() {
    super.initState();
    _startRound();
    _roundsCompleted = ProgressStore.instance.getMatchNameRounds();
    audioService.playAsset('assets/audio/match_name_voice.mp3');
  }
  ...
  Future<void> _onGuess(int index) async {
    ...
    if (isCorrect) {
      await ProgressStore.instance.incrementMatchNameRounds();
      if (!mounted) return;
      setState(() {
        _roundsCompleted = ProgressStore.instance.getMatchNameRounds();
      });
    ...
    showDialog<void>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text('Ready for the next one?'),
          actions: [
            TextButton(
              onPressed: () {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: const [
          SoundControls(helpAsset: 'assets/audio/match_name_voice.mp3'),
        ],
      ),
      body: AppBackground(
        child: Padding(
          ...
          child: Column(
            children: [
              Text(
                'Tap the animal that matches the name',
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed: () {
                        audioService.playAsset(_answer.nameAudioAsset);
                      },
